...
async function runBenchmark({ discovery, paths, options }) {
  ...
  for (const item of plan) {
    ...
    const compareOptions = {
      ...
      compareRunDir: path.join(runDir, "comparisons", safeSegment(item.model), item.exercise.id, `seed-${String(item.seed).padStart(3, "0")}`),
      brief: item.brief,
      dryRun: false,
    };
    const result = await runComparison({ exercise: item.exercise, discovery, paths, options: compareOptions });
    const row = benchmarkRowFromComparison({ item, result });
    rows.push(row);
  ...
  return { ok: true, status: "pass", run, rows, summary };
}
...
async function runStrategyComparison({ discovery, paths, options }) {
  ...
  for (const strategy of strategies) {
    ...
    const result = await runBenchmark({ discovery, paths, options: benchOptions });
    const strategyRun = {
  ...
  return { ok: true, status: "pass", run, rows, strategy_runs: strategyRuns, summary };
}
...
function summarizeBenchmarkRows(rows) {
  ...
  const mlabWins = completed.filter((row) => row.winner_source === "mlab").length;
  const directWins = completed.filter((row) => row.winner_source === "direct").length;
  const firstPassMlabWins = completed.filter((row) => row.first_pass_winner_source === "mlab").length;
  const firstPassDirectWins = completed.filter((row) => row.first_pass_winner_source === "direct").length;
  const scoreDeltas = completed.map((row) => row.score_delta).filter((value) => Number.isFinite(value));
  const firstPassScoreDeltas = completed.map((row) => row.first_pass_score_delta).filter((value) => Number.isFinite(value));
  ...
  return {
    ...
    first_pass_mlab_win_rate: total ? firstPassMlabWins / total : 0,
    first_pass_average_score_delta: average(firstPassScoreDeltas),
    mlab_wins: mlabWins,
    direct_wins: directWins,
    mlab_win_rate: total ? mlabWins / total : 0,
    direct_win_rate: total ? directWins / total : 0,
    average_score_delta: scoreDeltas.length ? scoreDeltas.reduce((sum, value) => sum + value, 0) / scoreDeltas.length : 0,
    mlab_pass_rate: total ? completed.filter((row) => row.mlab_pass).length / total : 0,
    direct_pass_rate: total ? completed.filter((row) => row.direct_pass).length / total : 0,
    copy_check_failures: completed.filter((row) => row.copy_check_ok === false).length,
    repair_rounds: completed.reduce((sum, row) => sum + Number(row.repair_rounds ?? 0), 0),
    repair_recoveries: completed.filter((row) => row.first_pass_winner_source === "direct" && row.winner_source === "mlab").length,
    invalid_direct_outputs: completed.filter((row) => row.invalid_direct_prose).length,
    invalid_candidate_outputs: completed.reduce((sum, row) => sum + Number(row.invalid_candidate_count ?? 0), 0),
    recovered_direct_outputs: completed.filter((row) => row.recovered_direct_prose).length,
    recovered_candidate_outputs: completed.reduce((sum, row) => sum + Number(row.recovered_candidate_count ?? 0), 0),
    known_usage: knownUsage,
...
function summarizeStrategies(rows, strategies) {
  return Object.fromEntries(strategies.map((strategy) => {
    ...
    const mlabWins = bucket.filter((row) => row.winner_source === "mlab").length;
    return [strategy.id, {
      label: strategy.label,
      settings: strategySettings(strategy),
      total,
      mlab_wins: mlabWins,
      direct_wins: bucket.filter((row) => row.winner_source === "direct").length,
      mlab_win_rate: total ? mlabWins / total : 0,
      first_pass_mlab_win_rate: total ? bucket.filter((row) => row.first_pass_winner_source === "mlab").length / total : 0,
      average_score_delta: average(bucket.map((row) => row.score_delta).filter((value) => Number.isFinite(value))),
      first_pass_average_score_delta: average(bucket.map((row) => row.first_pass_score_delta).filter((value) => Number.isFinite(value))),
      repair_rounds: bucket.reduce((sum, row) => sum + Number(row.repair_rounds ?? 0), 0),
      repair_recoveries: bucket.filter((row) => row.first_pass_winner_source === "direct" && row.winner_source === "mlab").length,
      invalid_direct_outputs: bucket.filter((row) => row.invalid_direct_prose).length,
      invalid_candidate_outputs: bucket.reduce((sum, row) => sum + Number(row.invalid_candidate_count ?? 0), 0),
      recovered_direct_outputs: bucket.filter((row) => row.recovered_direct_prose).length,
      recovered_candidate_outputs: bucket.reduce((sum, row) => sum + Number(row.recovered_candidate_count ?? 0), 0),
  ...
  return Object.fromEntries([...buckets.entries()].map(([exercise, bucket]) => [exercise, {
    ...
    strategies: Object.fromEntries([...new Set(bucket.map((row) => row.strategy))].map((strategy) => {
      ...
      return [strategy, {
        ...
        mlab_wins: strategyRows.filter((row) => row.winner_source === "mlab").length,
        average_score_delta: average(strategyRows.map((row) => row.score_delta).filter((value) => Number.isFinite(value))),
      }];
    })),
  }]));
}
...
function aggregateStrategyRows(rows) {
  ...
  return [...buckets.entries()].map(([strategy, bucket]) => {
    ...
    return {
      ...
      total,
      winner_source: mlabWins >= total - mlabWins ? "mlab" : "direct",
      mlab_wins: mlabWins,
      direct_wins: bucket.filter((row) => row.winner_source === "direct").length,
      mlab_win_rate: total ? mlabWins / total : 0,
      first_pass_mlab_win_rate: total ? bucket.filter((row) => row.first_pass_winner_source === "mlab").length / total : 0,
...
function recommendationConfidence(bucket, best, runnerUp) {
  ...
  if (bucket.length < strategyCount * 2) return "low";
  ...
  if (bucket.length >= strategyCount * 4 && (winGap >= 0.34 || deltaGap >= 3)) return "high";
...
function recommendationRationale(best, runnerUp) {
  const parts = [
    `${best.strategy_label} won ${best.mlab_wins}/${best.total} row(s) with average delta ${formatNumber(best.average_score_delta)} and known cost $${Number(best.known_usage?.cost ?? 0).toFixed(4)}.`,
  ];
  if (runnerUp) {
    parts.push(`Next best was ${runnerUp.strategy_label} at ${runnerUp.mlab_wins}/${runnerUp.total} row(s) and average delta ${formatNumber(runnerUp.average_score_delta)}.`);
  }
...
function summarizeBy(rows, key) {
  ...
  return Object.fromEntries([...buckets.entries()].map(([id, bucket]) => [id, {
    total: bucket.length,
    mlab_wins: bucket.filter((row) => row.winner_source === "mlab").length,
    direct_wins: bucket.filter((row) => row.winner_source === "direct").length,
    average_score_delta: average(bucket.map((row) => row.score_delta).filter((value) => Number.isFinite(value))),
  }]));
}
...
function writeBenchmarkReport(run, { rows, summary }) {
  const lines = [
    ...
    "## Rows",
    "",
    "| Exercise | Model | Seed | First | Final | MLab | Direct | Delta | Repairs | Copy |",
    "| --- | --- | ---: | --- | --- | ---: | ---: | ---: | ---: | --- |",
    ...rows.map((row) => [
      row.exercise,
      row.model,
      ...
      row.score_delta ?? "",
      row.repair_rounds,
      row.copy_check_ok === false ? "fail" : "pass",
    ].join(" | ")).map((line) => `| ${line} |`),
    "",
  ];
...
function writeStrategyReport(run, { rows, strategyRuns, summary }) {
  const lines = [
    ...
    "## Strategies",
    "",
    "| Strategy | Settings | MLab Wins | Win Rate | Avg Delta | Cost | Repairs | Repair Recoveries |",
    "| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |",
    ...Object.entries(summary.strategies).map(([id, item]) => markdownTableRow([
      id,
      strategySettingsLabel(item.settings),
      `${item.mlab_wins}/${item.total}`,
    ...
    "## Rows",
    "",
    "| Strategy | Exercise | Model | Seed | First | Final | Delta | Repairs | Cost |",
    "| --- | --- | --- | ---: | --- | --- | ---: | ---: | ---: |",
...
function emitBenchResult(result, options) {
  ...
  if (options.json) console.log(JSON.stringify(payload, null, 2));
    ...
    if (payload.run_dir) console.log(`Run: ${payload.run_dir}`);
    console.log(`MLab wins: ${payload.summary.mlab_wins}/${payload.summary.total}`);
...
function emitStrategiesResult(result, options) {
  ...
  if (options.json) console.log(JSON.stringify(payload, null, 2));
    ...
    for (const [strategy, item] of Object.entries(payload.summary.strategies ?? {})) {
      console.log(`${strategy}: ${item.mlab_wins}/${item.total} MLab wins, delta ${item.average_score_delta.toFixed(2)}`);
    }
  }
}
